...
void print_statistics(statistics_t* stats, const char* mode_name, double real_time)
{
    ...
    printf("  average time per file:     %.3f sec\n", avg_time);
                      ...
                      statistics_t* stats2, const char* name2, double time2)
{
    ...
    printf("| %-12s | %10.3f | %12.3f |\n", name1, time1, avg1);
    printf("| %-12s | %10.3f | %12.3f |\n", name2, time2, avg2);
    
    if (time2 > 0) {
        double speedup = time1 / time2;
        printf("Acceleration : %s faster %s for %.2f times\n", 
...
int main(int argc, char* argv[])
{
    ...
    if (is_auto) {
        ...
        char alt_output_dir[256];
        snprintf(alt_output_dir, sizeof(alt_output_dir), "%s_alt", output_dir);
        if (stat(alt_output_dir, &st) == -1) {
            mkdir(alt_output_dir, 0700);
        }
        
        statistics_t alt_stats;
        alt_stats.count = file_count;
        alt_stats.current_idx = 0;
        alt_stats.file_times = malloc(file_count * sizeof(double));
        alt_stats.file_names = malloc(file_count * sizeof(char*));
        alt_stats.file_success = malloc(file_count * sizeof(int));
        pthread_mutex_init(&alt_stats.mutex, NULL);
        
        job_context_t alt_ctx;
        alt_ctx.output_dir = alt_output_dir;
        alt_ctx.file_count = file_count;
        alt_ctx.current_index = 0;
        alt_ctx.completed_count = 0;
        alt_ctx.key = (uint8_t)key;
        alt_ctx.log_file = fopen("/dev/null", "w");
        alt_ctx.stats = &alt_stats;
        
        for (int i = 0; i < file_count; i++) {
            alt_ctx.input_files[i] = argv[file_start_index + i];
        }
        pthread_mutex_init(&alt_ctx.mutex, NULL);
        
        struct timespec alt_start, alt_end;
        clock_gettime(CLOCK_MONOTONIC, &alt_start);
        
        int alt_mode = (mode == 0) ? 1 : 0;
        
        if (alt_mode == 0) {
            process_sequential(&alt_ctx);
        } else {
            process_parallel(&alt_ctx);
        }
        
        clock_gettime(CLOCK_MONOTONIC, &alt_end);
        double alt_elapsed = (alt_end.tv_sec - alt_start.tv_sec) + 
                             (alt_end.tv_nsec - alt_start.tv_nsec) / 1e9;
        
        if (mode == 0) {
            print_comparison(&selected_stats, "sequential", total_elapsed,
                           &alt_stats, "parallel", alt_elapsed);
        } else {
            print_comparison(&alt_stats, "sequential", alt_elapsed,
                           &selected_stats, "parallel", total_elapsed);
        }
        
        for (int i = 0; i < alt_stats.count; i++) {
            if (alt_stats.file_names[i]) free(alt_stats.file_names[i]);
        }
        free(alt_stats.file_times);
        free(alt_stats.file_names);
        free(alt_stats.file_success);
        pthread_mutex_destroy(&alt_stats.mutex);
        if (alt_ctx.log_file && alt_ctx.log_file != stderr) {
            fclose(alt_ctx.log_file);
        }
        pthread_mutex_destroy(&alt_ctx.mutex);
    }
